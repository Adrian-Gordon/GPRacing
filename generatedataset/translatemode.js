...
const globalMode = nconf.get("mode")
        ...
        if(horse){
            ...
            if(performanceArray.length > 5){
              ...
              try{
                ...
                const modeTranslation = globalMode - mode
                //console.log(mode + " " + modeTranslation)
                const translated = predictions.map( x => x + modeTranslation)
                //console.log(JSON.stringify(predictions))
                //console.log(JSON.stringify(translated))

                translated.map(x => process.stdout.write(x + ","))
